...
uint16_t loadRawBitmapEeprom(uint32_t __xdata addr, uint16_t x, uint16_t y, bool color) {
    uint8_t img_dims[2];
    epdDeselect();
    eepromRead(addr, (void*)img_dims, 2);
    epdSelect();

    uint16_t xsize = img_dims[0] / 8;
    if (img_dims[0] % 8) xsize++;

    uint16_t ysize = img_dims[1];
    if(ysize == 0xff) ysize = 296;

    uint16_t size = xsize * ysize;

    pr("EEPROM BMP@ %lu Width: %d, Height: %d, size: %d\n", addr, img_dims[0], ysize, size);

    y -= ysize;

    setWindowX(x, x + (xsize * 8));
    setWindowY(y, ysize + y);
    setPosXY(x, y);
    shortCommand1(CMD_DATA_ENTRY_MODE, 3);
    if (color) {
        commandBegin(CMD_WRITE_FB_RED);
